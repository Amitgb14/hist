...
class Hist:
    ...
    def read_config(self):
        try:
            with open(HOST_CFG) as fread:
                ...
                ip_address = [ip.strip() for ip in datas
    ...
    def copy_node_key(self, node_ip):
        ...
        if status:
            ...
            return False
        self.histlog.info("Copied public ssh key {}".format(node_ip))
        return True

    def add_node(self, node_ip):
        if os.path.exists(HOST_CFG):
            if node_ip in self.read_config():
                self.histlog.info("Node already in record")
                return
        ...
        try:
            ...
            self.histlog.info("Successfully added {}".format(node_ip))
    ...
    def start_sync(self, node_ip):
        ...
        if status:
            ...
            return False
        self.histlog.info("Sync done : {}".format(node_ip))
        self.history = stdout
